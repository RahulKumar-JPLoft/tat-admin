...
import { tradieSubscription } from "redux/actions/action";
...
const SubscriptionTradie = (props) => {
  const Id = props?.location.state?.Id;
  const dispatch = useDispatch();
  const tradie_id = `user_id=${Id}`;

  useEffect(() => {
    if (Id) {
      dispatch(tradieSubscription(tradie_id));
    }
  }, [Id]);

  const { tradie_subscription_data } = useSelector(
  ...
  let sum = () => {
    let total = 0;
    tradie_subscription_data?.map((val) => {
      total += Number(val.amount);
    });
    return total;
  };

  // for (let i = 0; i < tradie_subscription_data.length; i++) {
  //   sum += tradie_subscription_data[i]?.amount;
  // }
        ...
        <Grid container spacing={2}>
          <Grid item xs={5} sm={2} md={2} ml={2}>
            <input
              ...
              value={`Total amount recieved: $${sum().toFixed(2)}`}
              style={{
...
export default SubscriptionTradie;
